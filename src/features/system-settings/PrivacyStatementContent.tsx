...
const initialData: PrivacyStatement[] = [
  { id: '1', title: '隐私声明', contentType: 'detail', content: '我们非常重视您的个人隐私，将按照法律法规保护您的个人信息。', createdAt: '2025-04-29 11:15:14', updatedAt: '2025-04-29 11:15:14', status: true, scope: '3个渠道' },
  { id: '2', title: '用户协议', contentType: 'link', content: 'https://www.iflytek.com/agreement', createdAt: '2025-04-28 09:30:00', updatedAt: '2025-04-28 09:30:00', status: false, scope: '2个产品' },
];
...
export default function PrivacyStatementContent() {
  ...
  const [editingItem, setEditingItem] = useState<PrivacyStatement | null>(null);
  ...
  const toggleSelect = (id: string, checked: boolean) => setSelectedIds(prev => checked ? [...prev, id] : prev.filter(x => x !== id));
  const toggleSelectAll = (checked: boolean) => setSelectedIds(checked ? filtered.map(d => d.id) : []);
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-hairline px-5 py-4">
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2 text-[13px] text-slate-500">
                <span className="whitespace-nowrap">隐私声明标题:</span>
                <input type="text" placeholder="请输入隐私声明标题" value={searchKeyword} onChange={e => setSearchKeyword(e.target.value)}
        ...
        <div className="fixed inset-0 z-[130] flex items-start justify-center overflow-y-auto bg-slate-900/40 px-4 pb-8 pt-[10vh] backdrop-blur-[3px]" onClick={() => setDialog(null)}>
          <div className="w-full max-w-[560px] overflow-hidden rounded-3xl bg-white shadow-[0_30px_80px_rgba(15,23,42,0.22)]" onClick={e => e.stopPropagation()}>
            <header className="flex items-center justify-between border-b border-hairline px-6 py-4">
              <h2 className="text-[16px] font-bold text-slate-800">{dialog === 'add' ? '新增声明' : dialog === 'edit' ? '编辑声明' : '查看声明'}</h2>
            ...
            <div className="space-y-4 px-6 py-5">
              <label className="block"><span className="text-[13px] font-medium text-slate-700">隐私声明标题</span>
              ...
              <label className="block"><span className="text-[13px] font-medium text-slate-700">内容类型</span>
                <select defaultValue={editingItem?.contentType ?? 'detail'} disabled={dialog === 'view'} className="mt-1 w-full rounded-xl border border-hairline bg-slate-50/60 px-3 py-2 text-[13px] outline-none"><option value="detail">详细内容</option><option value="link">链接</option></select>
              </label>
              <label className="block"><span className="text-[13px] font-medium text-slate-700">内容</span>
